...
const CrosswordComplete = () => {
  const { gameState, resetGame } = useContext(CrosswordContext);
  ...
  const calculateTimeTaken = () => {
    ...
    const timeDiff = Math.floor((gameState.endTime - gameState.startTime) / 1000); // in seconds
  ...
  const handleShareScore = () => {
    if (navigator.share) {
      navigator.share({
        title: 'My Crossword Challenge Score',
        text: `I completed the "${gameState.crosswordData.name}" crossword puzzle in ${calculateTimeTaken()}! Can you beat me?`,
        url: window.location.href,
      });
    } else {
      // Fallback for browsers that don't support Web Share API
      const shareText = `I completed the "${gameState.crosswordData.name}" crossword puzzle in ${calculateTimeTaken()}! Can you beat me?`;
  ...
  return (
    <div className="crossword-complete" style={{ backgroundColor: theme.cardBackground }}>
      ...
      <h2 style={{ color: theme.text }}>Puzzle Complete!</h2>
      ...
      <div className="score-container">
        <div className="score-box" style={{ backgroundColor: theme.secondary }}>
          ...
          <span className="score-value" style={{ color: theme.buttonText }}>{calculateTimeTaken()}</span>
        </div>
        
        <div className="score-box" style={{ backgroundColor: theme.success }}>
          <span style={{ color: theme.buttonText }}>Hints Used</span>
          <span className="score-value" style={{ color: theme.buttonText }}>{3 - gameState.hints}/3</span>
        </div>
      </div>
      
      <div className="puzzle-name" style={{ backgroundColor: theme.primary, color: theme.buttonText }}>
        <span>{gameState.crosswordData ? gameState.crosswordData.name : ''}</span>
      </div>
